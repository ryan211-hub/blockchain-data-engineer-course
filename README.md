...
- 当前 Lesson：Module 10 第 3 课｜Topic、Partition、Offset：数据流如何被切分和定位
- 当前状态：Module 10 第 2 课已完成并通过理解检查；准备进入第 3 课
- 最近完成：Module 10 第 2 课《Kafka 在实时 Pipeline 中到底解决什么问题》理解检查通过；Canonical Lesson Content 已同步到 Notion 和 GitHub。
- 最近掌握重点：已理解 Kafka 在 Blockchain Realtime Pipeline 中的系统角色：WebSocket 主要解决实时 Push；Kafka 作为 Durable Event Log 提供 Buffer、Decoupling、Fan-out 与 Replay。理解 Indexer 应形成 Canonical Event，而不应直接承担所有下游系统的调用与故障管理。
